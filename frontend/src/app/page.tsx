...
async function rpcWrite(method: string, args: any[], pk: string): Promise<string> {
  const r = await fetch(RPC, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jsonrpc: '2.0', method: 'gen_sendTransaction', params: [{ to: CONTRACT, data: { method, args }, value: '0x0' }, pk], id: Date.now() }) });
  return (await r.json())?.result;
}
...
function Card({ m, onBet, onResolve, isDemo }: {
  m: Market & { id: number };
  onBet: (m: Market) => void;
  ...
  return (
    <div
      style={{ background: 'linear-gradient(145deg,#1a1f35 0%,#141828 100%)', border: '1px solid rgba(255,255,255,.07)', borderRadius: '16px', padding: '22px', position: 'relative', overflow: 'hidden', transition: 'transform .2s,box-shadow .2s' }}
    ...
    >
      ...
      {resolved && m.resolution_reason && (
        <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)', borderRadius: '10px', padding: '10px 12px', fontSize: '10px', color: 'rgba(255,255,255,.5)', lineHeight: '1.7', marginBottom: '0' }}>
      ...
      {!resolved && (
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          {!isDemo && <button onClick={() => onResolve(m.id)} style={{ padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(129,140,248,.4)', background: 'rgba(129,140,248,.1)', color: '#818CF8', fontFamily: 'monospace', fontSize: '10px', cursor: 'pointer' }}>Resolve ⬡</button>}
        </div>
      )}
    </div>
  );
...
export default function App() {
  ...
  const load = useCallback(async () => {
    if (useDemo) { setMarkets(DEMOS.map((m, i) => ({ ...m, id: i }))); return; }
    try {
      const raw = await rpcCall('get_all_markets');
      if (raw) setMarkets(JSON.parse(raw));
    } catch { setUseDemo(true); setMarkets(DEMOS.map((m, i) => ({ ...m, id: i }))); }
  }, [useDemo]);

  useEffect(() => { load(); }, [load]);

  async function doAction(fn: () => Promise<void>) {
    if (!pk && !useDemo) { setWalletModal(true); return; }
    setLoading(true); setError('');
    try { await fn(); } catch (e: any) { setError(e.message || 'Transaction failed'); }
  ...
  async function handleBet() {
    if (!betModal) return;
    if (useDemo) {
      setMarkets(prev => prev.map(m => m.id !== betModal.id ? m : {
        ...m,
        total_yes: betSide === 'yes' ? m.total_yes + (parseInt(betAmount) || 0) : m.total_yes,
        total_no: betSide === 'no' ? m.total_no + (parseInt(betAmount) || 0) : m.total_no,
        bets: [...m.bets, { bettor: bettor || 'You', side: betSide, amount: parseInt(betAmount) || 0 }],
      }));
      setBetModal(null);
      return;
    }
    await doAction(async () => {
      ...
      const hash = await rpcWrite('place_bet', [betModal.id, bettor || 'anon', betSide, parseInt(betAmount)], pk);
      setTxHash(hash);
  ...
  async function handleResolve(id: number) {
    await doAction(async () => {
      setLoadingMsg('Fetching live data across 5 validators...');
      const hash = await rpcWrite('resolve_market', [id], pk);
      setTxHash(hash);
      setLoadingMsg('AI consensus in progress (30s-3min)...');
      await waitTx(hash);
      await load();
    });
  }
  ...
  async function handleCreate() {
    ...
    if (useDemo) {
      ...
      setCQ(''); setCUrl(''); setCDate(''); setCCreator('');
      setTab('markets');
      return;
    }
    await doAction(async () => {
      setLoadingMsg('Creating market on-chain...');
      const hash = await rpcWrite('create_market', [cQ, cCat, cUrl, cDate, cCreator], pk);
      setTxHash(hash);
      await waitTx(hash);
      await load();
      setTab('markets');
        ...
        <header style={{ borderBottom: '1px solid rgba(255,255,255,.06)', padding: '0 32px', background: 'rgba(15,17,23,.85)', backdropFilter: 'blur(24px)', position: 'sticky', top: 0, zIndex: 100 }}>
          <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '68px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              {useDemo && <span style={{ fontSize: '9px', background: 'rgba(245,158,11,.15)', color: '#F59E0B', border: '1px solid rgba(245,158,11,.3)', borderRadius: '6px', padding: '3px 10px', fontWeight: 600 }}>DEMO</span>}
              <button onClick={() => setWalletModal(true)} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '8px 16px', background: pkSaved ? 'rgba(52,211,153,.1)' : 'rgba(245,158,11,.1)', border: '1px solid ' + (pkSaved ? 'rgba(52,211,153,.3)' : 'rgba(245,158,11,.3)'), borderRadius: '10px', color: pkSaved ? '#34D399' : '#F59E0B', fontFamily: 'monospace', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>
                <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: pkSaved ? '#34D399' : '#F59E0B', animation: 'pulse 2s infinite', display: 'inline-block' }} />
                {pkSaved ? 'Connected' : 'Connect Wallet'}
              </button>
            </div>
          </div>
        </header>
        ...
        <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 24px' }}>
          ...
          {tab === 'markets' && (
            <div style={{ animation: 'fadeup .3s ease' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '28px' }}>
                {[
                  { label: 'Open Markets', value: openCount, icon: '📈', color: '#F59E0B' },
                  { label: 'Total Volume', value: fmtVol(totalVol), icon: '💰', color: '#34D399' },
                  { label: 'AI Resolved', value: markets.filter(m => m.status === 'resolved').length, icon: '⬡', color: '#818CF8' },
                  { label: 'Categories', value: 5, icon: '🗂', color: '#FB7185' },
                ].map(s => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(340px,1fr))', gap: '16px' }}>
                {filtered.map(m => <Card key={m.id} m={m} onBet={setBetModal} onResolve={handleResolve} isDemo={useDemo} />)}
                {!filtered.length && (
          ...
          {tab === 'create' && (
            <div style={{ maxWidth: '580px', margin: '0 auto', animation: 'fadeup .3s ease' }}>
              <h1 style={{ fontFamily: "'Fraunces',serif", fontSize: '34px', fontWeight: 700, marginBottom: '8px', letterSpacing: '-1px' }}>
                Create a <span style={{ background: 'linear-gradient(135deg,#F59E0B,#FB7185)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Market</span>
              </h1>
              ...
              <div style={{ background: 'linear-gradient(145deg,#1a1f35,#141828)', border: '1px solid rgba(255,255,255,.07)', borderRadius: '16px', padding: '28px' }}>
                ...
                {[
                  { label: 'Your Name / Address', val: cCreator, set: setCCreator, ph: '0x... or your name' },
                  { label: 'Resolution URL', val: cUrl, set: setCUrl, ph: 'https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd' },
                  { label: 'Resolution Date', val: cDate, set: setCDate, ph: '2025-07-01' },
                ].map(f => (
                  <div key={f.label}>
          ...
          {tab === 'resolve' && (
            <div style={{ maxWidth: '680px', margin: '0 auto', animation: 'fadeup .3s ease' }}>
              <h1 style={{ fontFamily: "'Fraunces',serif", fontSize: '34px', fontWeight: 700, marginBottom: '8px', letterSpacing: '-1px' }}>
                AI <span style={{ background: 'linear-gradient(135deg,#818CF8,#FB7185)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Resolution</span>
              </h1>
              ...
              {markets.filter(m => m.status === 'open').map(m => {
                ...
                return (
                  <div key={m.id} style={{ background: 'linear-gradient(145deg,#1a1f35,#141828)', border: '1px solid rgba(255,255,255,.07)', borderRadius: '12px', padding: '16px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <button onClick={() => handleResolve(m.id)} disabled={loading || useDemo} style={{ padding: '9px 18px', borderRadius: '9px', border: '1px solid rgba(129,140,248,.4)', background: 'rgba(129,140,248,.1)', color: useDemo ? 'rgba(255,255,255,.2)' : '#818CF8', fontFamily: 'monospace', fontSize: '10px', cursor: useDemo ? 'not-allowed' : 'pointer', fontWeight: 600 }}>
                      {loading ? '...' : useDemo ? 'Connect wallet' : 'Resolve ⬡'}
                    </button>
                  </div>
                );
              })}
          ...
          {tab === 'portfolio' && (
            <div style={{ maxWidth: '680px', margin: '0 auto', animation: 'fadeup .3s ease' }}>
              <h1 style={{ fontFamily: "'Fraunces',serif", fontSize: '34px', fontWeight: 700, marginBottom: '8px', letterSpacing: '-1px' }}>
                Your <span style={{ background: 'linear-gradient(135deg,#34D399,#818CF8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Positions</span>
              </h1>
              <p style={{ fontSize: '12px', color: 'rgba(255,255,255,.4)', marginBottom: '24px' }}>Enter your name or address to see all your bets.</p>
              <div style={{ background: 'linear-gradient(145deg,#1a1f35,#141828)', border: '1px solid rgba(255,255,255,.07)', borderRadius: '14px', padding: '18px', marginBottom: '20px' }}>
                <input type="text" value={bettor} onChange={e => setBettor(e.target.value)} placeholder="0x... or your name" style={{ width: '100%', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.1)', borderRadius: '10px', padding: '11px 14px', color: '#F1F5F9', fontFamily: 'monospace', fontSize: '12px', outline: 'none' }} />
              </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '20px' }} onClick={() => setWalletModal(false)}>
          <div style={{ background: 'linear-gradient(145deg,#1e2340,#161b2e)', border: '1px solid rgba(255,255,255,.1)', borderRadius: '20px', padding: '32px', maxWidth: '420px', width: '100%', animation: 'fadeup .2s ease' }} onClick={e => e.stopPropagation()}>
            <div style={{ textAlign: 'center' as const, marginBottom: '24px' }}>
              ...
              <div style={{ fontSize: '12px', color: 'rgba(255,255,255,.4)', lineHeight: 1.7 }}>Enter your testnet private key to place bets and resolve markets on GenLayer.</div>
            </div>
            <div style={{ background: 'rgba(245,158,11,.08)', border: '1px solid rgba(245,158,11,.2)', borderRadius: '10px', padding: '12px 14px', fontSize: '10px', color: 'rgba(245,158,11,.8)', marginBottom: '20px', lineHeight: 1.7 }}>
              ⚠ Use a testnet-only key. Never use a mainnet private key in a browser.
            </div>
            <label style={{ display: 'block', fontSize: '9px', color: 'rgba(255,255,255,.3)', letterSpacing: '2px', textTransform: 'uppercase' as const, marginBottom: '8px' }}>Private Key</label>
            <input type="password" value={pk} onChange={e => setPk(e.target.value)} placeholder="0x..." style={{ width: '100%', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)', borderRadius: '10px', padding: '12px 14px', color: '#F1F5F9', fontFamily: 'monospace', fontSize: '13px', outline: 'none', marginBottom: '16px' }} />
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setWalletModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '1px solid rgba(255,255,255,.1)', background: 'transparent', color: 'rgba(255,255,255,.5)', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: '13px', cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => { setPkSaved(true); setUseDemo(false); setWalletModal(false); }} style={{ flex: 2, padding: '12px', borderRadius: '12px', border: 'none', background: 'linear-gradient(135deg,#F59E0B,#EF4444)', color: '#fff', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>Connect</button>
            </div>
            <div style={{ textAlign: 'center' as const, marginTop: '16px' }}>
              <button onClick={() => { setUseDemo(true); setPkSaved(false); setWalletModal(false); }} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,.3)', fontSize: '11px', cursor: 'pointer', textDecoration: 'underline' }}>Continue in Demo Mode</button>
            </div>
          </div>
        </div>
      )}
      ...
      {betModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '20px' }} onClick={() => setBetModal(null)}>
          <div style={{ background: 'linear-gradient(145deg,#1e2340,#161b2e)', border: '1px solid rgba(255,255,255,.1)', borderRadius: '20px', padding: '28px', maxWidth: '400px', width: '100%', animation: 'fadeup .2s ease' }} onClick={e => e.stopPropagation()}>
            ...
            <label style={{ display: 'block', fontSize: '9px', color: 'rgba(255,255,255,.3)', letterSpacing: '2px', textTransform: 'uppercase' as const, marginBottom: '8px' }}>Your Name / Address</label>
            <input type="text" value={bettor} onChange={e => setBettor(e.target.value)} placeholder="0x... or your name" style={{ width: '100%', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.1)', borderRadius: '10px', padding: '11px 14px', color: '#F1F5F9', fontFamily: 'monospace', fontSize: '12px', outline: 'none', marginBottom: '16px' }} />
            <button onClick={handleBet} disabled={loading} style={{ width: '100%', padding: '13px', borderRadius: '12px', border: 'none', background: betSide === 'yes' ? 'linear-gradient(135deg,#34D399,#059669)' : 'linear-gradient(135deg,#FB7185,#E11D48)', color: '#fff', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
              Bet {betAmount} on {betSide.toUpperCase()}
            </button>
          </div>
        </div>
      )}
